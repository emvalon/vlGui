...
static void
vlGui_menuDrawLeadingLine(vlGui_menu_t *menu, vlGui_menuEntry_t *entry,
                          int16_t lineHight, int16_t origin)
{
    uint8_t childLevel;
    int16_t bottom;
    int16_t x, dy;
    ...
    while (next) {
        if (next->level == childLevel) {
            ...
            if (next->folded && vlGui_menuEntryHasChildren(next)) {
                vlGui_drawLine(win, x, bottom - 2, x + 3, bottom - 2, 1, 
                               VLGUI_COLOR_WHITE);
                vlGui_drawLine(win, x, bottom - 4, x + 3, bottom - 4, 1, 
                               VLGUI_COLOR_WHITE);
            }
...
vlGui_menuDrawBox(vlGui_menu_t *menu, int16_t y, vlGui_menuEntry_t *entry)
{
    ...
    vlGui_drawRectangle(win, x, y + VLGUI_MENU_LINE_SPACING, s, s, 
                        VLGUI_COLOR_WHITE);
...
vlGui_menuDraw(vlGui_window_t *win, uint8_t flag)
{
    ...
    if (menu->flags & VLGUI_MENU_FLAGS_MOVE_CHILD) {
        vlGui_drawBlock(win, VLGUI_MENU_ENTRY_MARGIN + VLGUI_MENU_ENTRY_PADDING 
                        + (VLGUI_MENU_ENTRY_RETRACE * entrySelected->level), 
                        ySelected + VLGUI_MENU_LINE_SPACING, win->win_width - 2,
                        font->fontHeight, VLGUI_COLOR_BLACK);
    }
    vlGui_drawString(win, VLGUI_MENU_ENTRY_MARGIN + VLGUI_MENU_ENTRY_PADDING 
                     + (VLGUI_MENU_ENTRY_RETRACE * entrySelected->level), 
                     ySelected + VLGUI_MENU_LINE_SPACING, entrySelected->str, 
                     VLGUI_COLOR_WHITE);
    /* Draw the selected bar. We may cover the child entries' text. */
    x = VLGUI_MENU_ENTRY_MARGIN + menu->selectedRectRetract;
    vlGui_drawBlock(win, x, rectOffset, win->win_width - 2 - 
                    (2 * VLGUI_MENU_ENTRY_MARGIN) - menu->selectedRectRetract,
    ...
    vlGui_drawRectangle(win, win->win_width - 3, -offset, 
                          2, unit, VLGUI_COLOR_WHITE);
}
...
vlGui_menuEngineMoveChildCb(vlGui_menu_t *menu, int16_t delta)
{
    ...
    if (!vlGui_engineIsRunning(&menu->mainEngine)) {
        ...
        menu->flags &= ~(VLGUI_MENU_FLAGS_FOLD_CHILDREN | 
                         VLGUI_MENU_FLAGS_MOVE_CHILD);
    } else {
        if (menu->flags & VLGUI_MENU_FLAGS_FOLD_CHILDREN) {
...
vlGui_menuProcessKeyMoveSelect(vlGui_menu_t *menu, bool moveForward)
{
    ...
    while (1) {
        ...
        if (!sel) {
            ...
            vlGui_engineStop(&menu->mainEngine);
            vlGui_engineStop(&menu->auxEngine);  
            menu->selEntry = sel;
...
vlGui_menuProcessKeyFold(vlGui_menu_t *menu)
{
    ...
    if (refresh) {
        ...
        if (fold) {
             ...
             * will stop scrolling.
             */
            ref = winHight - ((sel->childrenNum + 1) * 
                  menu->lineHight);
            if (ref < 0) {
                ref = 0;
            }
...
vlGui_menu_t * 
vlGui_menuCreate(vlGui_window_t *parent, int16_t x, int16_t y,     
                     int16_t width, uint16_t height)
{
    vlGui_menu_t *menu;
    VLGUI_ASSERT(parent);

    menu = (vlGui_menu_t *)vlGui_windowCreate(parent, x, y,
    ...
    vlGui_engineInit(&menu->mainEngine, vlGui_engineCurveOutQuint,
                     vlGui_menuMainEngineCb, menu);
    vlGui_engineInit(&menu->auxEngine, vlGui_engineCurveOutQuint,
                     vlGui_menuAuxEngineCb, menu);
...
void
vlGui_menuAddEntry(vlGui_menu_t *menu, vlGui_menuEntry_t *parent,
