...
vlGui_drawIconMenu(vlGui_window_t *win, uint8_t flag)
{
    ...
    y = 0;
    vlGui_drawLine(
        win, x, y, x + VLGUI_ICONMENU_SEL_LEN, y, VLGUI_ICONMENU_SEL_WID, VLGUI_COLOR_WHITE);
    vlGui_drawLine(
        win, x, y, x, y + VLGUI_ICONMENU_SEL_LEN, VLGUI_ICONMENU_SEL_WID, VLGUI_COLOR_WHITE);
    ...
    vlGui_drawLine(win,
                   x,
                   y,
                   x - VLGUI_ICONMENU_SEL_LEN + VLGUI_ICONMENU_SEL_WID,
                   y,
                   VLGUI_ICONMENU_SEL_WID,
                   VLGUI_COLOR_WHITE);
    vlGui_drawLine(win,
                   x,
                   y + 1,
                   x,
                   y - VLGUI_ICONMENU_SEL_LEN + VLGUI_ICONMENU_SEL_WID,
                   VLGUI_ICONMENU_SEL_WID,
                   VLGUI_COLOR_WHITE);
    ...
    vlGui_drawBlock(
        win, 0, y, 8 - (6.0 * menu->strOffset / strPos), font->fontHeight, VLGUI_COLOR_WHITE);
    ...
    vlGui_drawString(win,
                     VLGUI_STR_CENTER_X(win->win_width, font, 4),
                     menu->strOffset + y,
                     selItem->name,
                     VLGUI_COLOR_WHITE);
}
...
vlGui_iconMenuStrEngineCb(void *param, int16_t delta)
{
    ...
    if (!vlGui_engineIsRunning(&menu->strEngine) && menu->strOffset) {
        ...
        vlGui_engineStart(&menu->strEngine, -menu->strOffset, VLGUI_ICONMENU_MOVE_DUR >> 1);
    }
}
                     ...
                     uint8_t iconMAxSize)
{
    ...
    menu = (vlGui_iconMenu_t *)vlGui_windowCreate(
    ...
    vlGui_engineInit(&menu->engine, vlGui_engineCurveOutQuint, vlGui_iconMenuEngineCb, menu);
    vlGui_engineInit(&menu->strEngine, vlGui_engineCurveOutQuint, vlGui_iconMenuStrEngineCb, menu);
